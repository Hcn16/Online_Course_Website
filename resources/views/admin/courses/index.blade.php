...
<div class="content-wrapper">
    <!-- Content Header (Page header) -->
     @if(isset($is_detail) && $is_detail == 1)
        @include('partials.content-header', ['name' => 'Courses_Created', 'key' => 'List'])

    @else
    @include('partials.content-header', ['name' => 'Courses', 'key' => 'List'])
    ...
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <a href="{{route('courses.create')}}" class="btn btn-success float-right m-2">Add</a>
                </div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$course->name}}</td>
                                <td>
                                    @foreach($course->course_teacher as $teacher)
                                        <p>{{$teacher->name}}</p></br>
                                        
                                    @endforeach

                                </td>
                                <td>{{$course->num_learner_of_course($course->id)}}</td>
                               <td>{{$course->cost}}</td>
